...
describe('Notification Service E2E Tests', () => {
  ...
  describe('Allocation Confirmed Event', () => {
    it('should process allocation_confirmed event and send email', async () => {
      const event = TestFixtures.allocationConfirmedEvent();

      console.log('📤 Sending ALLOCATION_CONFIRMED event...');
      ...
      expect(event.visitId).toBeDefined();
      expect(event.shippingCompanyEmail).toContain('@');
    }, 15000);

    it('should include correct data in allocation email', async () => {
      const event = TestFixtures.allocationConfirmedEvent();

      console.log('📤 Sending ALLOCATION_CONFIRMED event with verification...');
      await kafkaHelper.sendEvent(KAFKA_EVENTS.ALLOCATION_CONFIRMED, event);

      await new Promise(resolve => setTimeout(resolve, 2000));

      // Verify event data
      expect(event).toHaveProperty('vesselName');
      expect(event).toHaveProperty('berthName');
      expect(event).toHaveProperty('lockExpiry');
      expect(event.berthId).toMatch(/^BERTH-\d+$/);
    }, 15000);
  });
  ...
  describe('Payment Confirmed Event', () => {
    it('should process payment_confirmed event', async () => {
      ...
      console.log('📤 Sending PAYMENT_CONFIRMED event...');
      ...
      console.log('✓ Payment confirmed event processed');
    }, 15000);

    it('should include transaction details in payment email', async () => {
      const event = TestFixtures.paymentConfirmedEvent();

      console.log('📤 Sending PAYMENT_CONFIRMED event with details...');
      await kafkaHelper.sendEvent(KAFKA_EVENTS.PAYMENT_CONFIRMED, event);

      await new Promise(resolve => setTimeout(resolve, 2000));

      expect(event).toHaveProperty('transactionId');
      expect(event).toHaveProperty('confirmedAt');
      expect(event.amount).toEqual(expect.any(Number));
    }, 15000);
  });

  describe('Payment Failed Event', () => {
    it('should process payment_failed event', async () => {
      const event = TestFixtures.paymentFailedEvent();

      console.log('📤 Sending PAYMENT_FAILED event...');
      await kafkaHelper.sendEvent(KAFKA_EVENTS.PAYMENT_FAILED, event);
    ...
    it('should include failure reason in email', async () => {
      ...
      console.log('📤 Sending PAYMENT_FAILED event with reason...');
      await kafkaHelper.sendEvent(KAFKA_EVENTS.PAYMENT_FAILED, event);
